...
export const syncRouter = router({
  ...
  importLegacy: protectedProcedure.input(z.object({
    ...
    for (const item of input.projects) await db.insert(projects).values({ id: item.id, ownerId: ctx.user.id, goalId: item.goalId ?? null, title: item.title, description: item.description ?? null, color: item.color ?? "#7FB5D6", entityStatus: item.status ?? "active" as const, visibility: item.visibility ?? "private" as const }).onDuplicateKeyUpdate({ set: { goalId: item.goalId ?? null, title: item.title, description: item.description ?? null, color: item.color ?? "#7FB5D6", entityStatus: item.status ?? "active" as const, updatedAt: new Date() } });
