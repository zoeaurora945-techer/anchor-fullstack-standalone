...
export const syncRouter = router({
  ...
  importLegacy: protectedProcedure.input(z.object({
    ...
    const db = await requireDb();
    await ensureProfile(ctx.user);
    for (const item of input.goals) await db.insert(goals).values({ id: item.id, ownerId: ctx.user.id, title: item.title, description: item.description ?? null, color: item.color ?? "#6EA8FE", status: item.status ?? "active", visibility: item.visibility ?? "private" }).onDuplicateKeyUpdate({ set: { title: item.title, description: item.description ?? null, color: item.color ?? "#6EA8FE", updatedAt: new Date() } });
    for (const item of input.projects) await db.insert(projects).values({ id: item.id, ownerId: ctx.user.id, goalId: item.goalId ?? null, title: item.title, description: item.description ?? null, status: item.status ?? "active", visibility: item.visibility ?? "private" }).onDuplicateKeyUpdate({ set: { goalId: item.goalId ?? null, title: item.title, description: item.description ?? null, updatedAt: new Date() } });
